...
// ── Customization formatting helpers ──────────────────────────────────────────
// Shared by the collapsed preview line and the expanded detail rows so the
// amount value + price adjustment are shown identically in both places.

/// The amount's literal value (e.g. 10 → "10", null/'' → ''). Trims a trailing
/// ".0" so 10.0 reads as "10".
String _fmtAmountValue(dynamic v) {
  if (v == null) return '';
  if (v is num) {
...
/// The signed price adjustment (e.g. +€1.50 / −€0.50). 0 → ''.
String _fmtPriceDelta(dynamic d) {
  final v = (d as num?)?.toDouble() ?? 0;
  if (v == 0) return '';
  final sign = v > 0 ? '+' : '−';
  ...
  String _buildPreviewLine() {
    ...
    for (final c in _custs.take(2)) {
      ...
      if (c['removed'] == true) {
        ...
        final valueStr = _fmtAmountValue(c['value']);
        final deltaStr = _fmtPriceDelta(c['price_delta']);
        var s = '$name: ${c['amount']}';
        if (valueStr.isNotEmpty) s += ' ·$valueStr';
        if (deltaStr.isNotEmpty) s += ' $deltaStr';
        parts.add(s);
      }
    }
...
class _CustomizationRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // The amount pill shows label + literal value together (e.g. "polu ·10")
    // so the cook sees exactly how much to use.
    final amountText = (amount != null && amount.isNotEmpty)
        ? (valueStr.isNotEmpty ? '$amount ·$valueStr' : amount)
        : '';
    ...
    return Padding(
      ...
      child: Wrap(
        ...
        children: [
          ...
          if (!removed && amountText.isNotEmpty)
            Container(
          ...
          if (!removed && deltaStr.isNotEmpty)
            Text(
              deltaStr,
              style: TextStyle(
                fontFamily: 'Nunito',
