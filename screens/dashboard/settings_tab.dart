            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  // ── Cash & Receipts ──────────────────────────────────────
const SectionHeader(title: 'CASH & RECEIPTS'),
const _CashReceiptsSection(),
const SizedBox(height: 4),
...
class _CashReceiptsSection extends StatefulWidget {
  const _CashReceiptsSection();

  @override
  State<_CashReceiptsSection> createState() => _CashReceiptsSectionState();
}

class _CashReceiptsSectionState extends State<_CashReceiptsSection> {
  bool _savingCash = false;

  Future<void> _toggleCash(bool value) async {
    setState(() => _savingCash = true);
    try {
      await context.read<BarProvider>().setAcceptsCash(value);
    } catch (e) {
      if (mounted) {
      ...
      if (mounted) setState(() => _savingCash = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return _SettingsCard(children: [
      // ── Cash toggle ──────────────────────────────────────────────
      Row(children: [
        const Icon(Icons.payments_rounded, color: Color(0xFF6A4FA3), size: 20),
        const SizedBox(width: 10),
        const Expanded(
          child: Text('Accept cash orders',
              style: TextStyle(
                  fontFamily: 'Nunito',
                  fontWeight: FontWeight.w800,
                  fontSize: 13,
        ...
        _savingCash
            ? const SizedBox(
                ...
                child: CircularProgressIndicator(strokeWidth: 2))
            : Switch(
                value: bar.acceptsCash,
                activeColor: const Color(0xFF6A4FA3),
                onChanged: _toggleCash,
              ),
      ]),
      const Text(
        'Customers can order now and pay the waiter in cash on delivery. '
        'Cash orders appear purple in the Orders tab.',
        style: TextStyle(
      ...
      Row(children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Automatic receipts (myDATA)',
                style: TextStyle(
                    fontFamily: 'Nunito',
                    fontWeight: FontWeight.w800,
                    fontSize: 13,
                    color: AppTheme.textPrimary)),
            Text(
              bar.receiptProvider != null
                  ? 'Connected · ${bar.receiptProvider} · ΑΦΜ ${bar.vatNumber ?? "—"}'
                  : 'Not connected — receipts issued by your own ταμειακή',
              style: TextStyle(
                  fontFamily: 'Nunito', fontSize: 11,
                  fontWeight: FontWeight.w700,
                  color: bar.receiptProvider != null
                      ? AppTheme.seafoam
        ...
        child: OutlinedButton.icon(
          ...
          label: Text(
              bar.receiptProvider != null
                  ...
                  : 'Connect receipt provider',
              style: const TextStyle(
                  fontFamily: 'Nunito', fontWeight: FontWeight.w700)),
        ),
      ),
      if (bar.receiptProvider != null) ...[
        const SizedBox(height: 8),
        Center(
          child: GestureDetector(
            onTap: () => _disconnect(context),
            child: const Text('Disconnect (use my own ταμειακή)',
                style: TextStyle(
                    fontFamily: 'Nunito', fontSize: 11,
  ...
  Future<void> _disconnect(BuildContext context) async {
    try {
      await context.read<BarProvider>().saveReceiptProvider(
          provider: null, vatNumber: null, credentials: null);
    } catch (e) {
      if (context.mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('Failed: $e')));
  ...
  void _showProviderDialog(BuildContext context) {
    final vatCtrl  = TextEditingController(
        text: context.read<BarProvider>().bar?.vatNumber ?? '');
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDlg) => AlertDialog(
          backgroundColor: Colors.white,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
          title: const Text('Epsilon Digital',
              style: TextStyle(
                  fontFamily: 'Nunito', fontWeight: FontWeight.w800)),
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Text(
                'Create an API user in your Epsilon Digital account, then '
                'enter the credentials below. Receipts will be issued and '
                'sent to myDATA automatically for every paid order.',
                style: TextStyle(
                    fontFamily: 'Nunito', fontSize: 12,
                    color: AppTheme.textSecondary),
              ),
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                    labelText: 'ΑΦΜ (9 digits)', counterText: ''),
              ),
              TextField(
                controller: userCtrl,
                decoration: const InputDecoration(labelText: 'API username'),
              ),
              TextField(
                controller: passCtrl,
                obscureText: true,
