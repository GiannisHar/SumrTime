...
class BarProvider extends ChangeNotifier {
  ...
  bar!.menu[catId]?.products.remove(prodId);
  notifyListeners();
}
  ...
  /// Cash delivery flow: confirm collection → mark paid on the server
/// (which also issues the receipt) → then mark delivered.
Future<Order?> collectCashAndDeliver(String orderId) async {
  var updated = await ApiService.collectCash(orderId);
  if (updated.receiptMark == null) {
    // receipt issuance is async server-side — give it a beat and refetch
    await Future.delayed(const Duration(milliseconds: 800));
    updated = await ApiService.collectCash(orderId); // idempotent — returns order
  }
 ...
 // ── Socket wiring (bar owner + staff) ────────────────────────────────────
  void _connectSocket(String barId) {
    ...
    // Fires on initial connect AND every reconnect. SocketService must
    // re-emit join_bar_room inside its own onConnect (rooms are lost on
    // reconnect), then we pull fresh state so nothing missed while
    // disconnected is lost.
    svc.onConnected(() => _resync());
  ...
  /// Refetch server truth after a (re)connect — covers events missed
  /// while offline. Orders always; menu only for owner sessions.
  Future<void> _resync() async {
    try {
      ...
      // If the JWT died while disconnected, loadOrders() 401s and
      // ApiService.handleSessionExpired() already kicked us to login.
    }
  }
...
Future<void> setAcceptsCash(bool v) async {
  await ApiService.setAcceptsCash(v);
  _bar!.acceptsCash = v;
  notifyListeners();
}

Future<void> saveReceiptProvider({
  required String? provider,
  required String? vatNumber,
  required Map<String, String>? credentials,
}) async {
  await ApiService.setReceiptProvider(provider, vatNumber, credentials);
  _bar!.receiptProvider = provider;
  _bar!.vatNumber = vatNumber;
  notifyListeners();
}
